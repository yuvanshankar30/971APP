<script>
  ...
  import { Plus, Save, Undo2, Redo2, RotateCw, RotateCcw, Download, Upload, Crosshair, MousePointer2, FolderOpen, Search, RefreshCw, Ruler, FileCode, Trash2, Copy, Settings, X, Pencil, ExternalLink, AlertTriangle, Cpu } from 'lucide-svelte';
  ...
  onMount(() => {
    ...
    const onKey = (event) => {
      ...
      if (event.key.toLowerCase() === 'e') { event.preventDefault(); emitCutId = activeCutId; emitSuffix = 'all'; showEmit = true; }
  ...
  function runCommand(command) {
    ...
    if (command === 'emit') { emitCutId = activeCutId; emitSuffix = 'all'; showEmit = true; return; }
  ...
  async function buildEmissions() {
    ...
    return targets.map(suffix => ({ ...emitNestingGcode({ name: emitName || sheet.name, placements: emitPlacements, programs: gcodePrograms, suffix, filenameSuffix: suffix || defaultGroupLabel, suffixCount: 2, dialect, thickness: sheet.thickness_key }), suffix })).filter(result => result.emitted);
  }
  ...
  async function commitGcode() {
    ...
    try {
      ...
      for (const result of results) {
        ...
        await recordEmission({ cut_id: emitCut?.id || activeCutId, suffix: result.suffix || '', dialect, output_storage_path: path, tool_order: [] });
      }
...
{#if showCamJobs}<div class="scrim" role="presentation" on:click|self={() => (showCamJobs = false)}><div class="modal cam-jobs-modal" role="dialog" tabindex="-1" aria-label="AutoCAM jobs"><button type="button" class="modal-close" title="Close" on:click={() => (showCamJobs = false)}><X size={18}/></button><h2>AutoCAM jobs</h2><div class="cam-job-filters"><label>Search<input aria-label="Search CAM jobs" placeholder="Search files" bind:value={camJobSearch}/></label><label>From<input type="date" bind:value={camJobDateFrom}/></label><label>To<input type="date" bind:value={camJobDateTo}/></label></div>{#if loadingCamJobs}<div class="library-loading" aria-live="polite"><span class="loading-spinner" aria-hidden="true"></span><span>Loading CAM jobs...</span></div>{:else if !visibleCamJobs.length}<p class="hint">No completed CAM jobs with G-code are available.</p>{:else}<div class="cam-job-list">{#each visibleCamJobs as job}<button type="button" class="cam-job-item" on:click={() => insertCamJob(job)}><strong>{job.name || job.gcode_file_name || "Untitled job"}</strong><span>{job.operation_type || "CAM"} · {job.created_at ? new Date(job.created_at).toLocaleString() : "unknown date"}</span></button>{/each}</div>{/if}</div></div>{/if}{#if showEmit}<div class="scrim"><form class="modal" on:submit|preventDefault={commitGcode}><button type="button" class="modal-close" title="Close" on:click={() => showEmit = false}><X size={18}/></button><h2>Emit G-code</h2><label>Cut<select bind:value={emitCutId} on:change={() => emitSuffix = "all"}>{#each sheet?.nesting_cuts || [] as cut}<option value={cut.id}>{cut.name}</option>{/each}</select></label><label>Program name<input bind:value={emitName}/></label><p class="hint">{programType === 'tap' ? 'WinCNC (.tap)' : '971 / LinuxCNC (.ngc)'}</p>{#if emitValidation.length}<div class="emit-validation"><AlertTriangle size={16}/><span>Fix {emitValidation.length} validation issue{emitValidation.length === 1 ? '' : 's'} before emitting.</span></div>{/if}<fieldset><legend>Program group</legend><label class="radio"><input type="radio" bind:group={emitSuffix} value="all"/> All available groups</label>{#each availableSuffixes as suffix}<label class="radio"><input type="radio" bind:group={emitSuffix} value={suffix}/> {suffix || defaultGroupLabel}</label>{/each}{#if !availableSuffixes.length}<p class="hint">Add a part or hole before emitting.</p>{/if}</fieldset><div class="row emit-actions"><button type="button" class="btn btn-secondary" on:click={() => showEmit = false}>Cancel</button><button type="button" class="btn btn-secondary" on:click={downloadGcode} disabled={!emitPlacements.length || !availableSuffixes.length || emitValidation.length}><Download size={16}/> Download</button><button class="btn btn-primary" disabled={!emitPlacements.length || !availableSuffixes.length || emitValidation.length}><Upload size={16}/> Commit to Output</button></div></form></div>{/if}
