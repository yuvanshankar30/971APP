...
{:else if screen === 'settings'}
  <main class="nesting"><header><div><p class="eyebrow">Sheet Nesting</p><h1>Settings</h1></div><a class="btn btn-secondary" href="/nesting">Back to sheets</a></header><section class="settings-panel"><h2>Coordinate system</h2><p>Sheets use positive inch dimensions. The lower-left of each sheet is X0 Y0; the canvas handles screen-space inversion internally.</p><h2>Part library</h2><p>Upload all <code>.ngc</code> or <code>.tap</code> variants for a part at once. Their final underscore suffixes become emission groups, matching JProg's suffix workflow.</p><h2>Hole programs</h2><p>Added holes use the bundled JProg thickness program and only emit with the <code>holes</code> suffix.</p><h2>Workflow boundary</h2><p>Nesting remains standalone. Emitting a program does not queue or update AutoCAM or Fusion.</p></section></main>
{:else if screen === 'select'}
  <main class="nesting"><header><div><p class="eyebrow">Manufacturing</p><h1>Sheet Nesting</h1></div><div class="header-actions"><button class="icon-button" title="Settings" on:click={() => screen = 'settings'}><Settings size={18}/></button><button class="btn btn-primary" on:click={() => showNewSheet = true}><Plus size={16}/> New sheet</button></div></header><label class="search"><Search size={17}/><input bind:value={sheetSearch} placeholder="Search sheets"/></label><section class="sheet-list">{#each visibleSheets as item}<button class="sheet-row" on:click={() => openSheet(item.id)}><strong>{item.name}</strong><span>{item.width_in} x {item.height_in} in · {item.thickness_key} in · {item.nesting_cuts?.length || 0} cuts</span></button>{:else}<p>No matching sheets.</p>{/each}</section></main>
{:else}
  <main class="workspace"><header class="workspace-header"><button class="icon-button" title="Back to sheets" on:click={() => { screen = 'select'; goto('/nesting'); }}><FolderOpen size={18}/></button><div><p class="eyebrow">{sheet?.name} · {sheet?.width_in} x {sheet?.height_in} in · {sheet?.thickness_key} in</p><h1>Sheet Nesting</h1></div><div class="header-actions"><button class="icon-button" title="Fit sheet" on:click={fitView}><Crosshair size={18}/></button><button class:active={measuring || measure.length} class="icon-button" title="Measure" on:click={() => { measuring = !measuring; if (measuring) measure = []; toastActions.show(measuring ? 'Click two points to measure' : 'Measurement cancelled'); }}><Ruler size={18}/></button><button class="icon-button" title="Undo" disabled={!undo.canUndo} on:click={undoChange}><Undo2 size={18}/></button><button class="icon-button" title="Redo" disabled={!undo.canRedo} on:click={redoChange}><Redo2 size={18}/></button><button class="btn btn-secondary" on:click={save}><Save size={16}/>{saving ? 'Saving' : 'Save'}</button><button class="btn btn-primary" on:click={() => { showEmit = true; emitSuffix = availableSuffixes.length === 1 ? availableSuffixes[0] : 'all'; }}><Download size={16}/> Emit G-code</button></div></header>
  <div class="workspace-body"><aside><section><label for="active-cut">Active cut</label><select id="active-cut" value={activeCutId} on:change={(e) => chooseCut(e.currentTarget.value)}>{#each sheet?.nesting_cuts || [] as cut}<option value={cut.id}>{cut.name}</option>{/each}</select><button class="text-button" on:click={addCut}><Plus size={15}/> Add cut</button></section><section><h2>Place</h2><label class="btn btn-secondary upload"><Upload size={16}/> Upload part program(s)<input type="file" accept=".ngc,.tap" multiple on:change={uploadParts}/></label><div class="row"><button class="btn btn-secondary" on:click={() => showLibrary = !showLibrary}><FolderOpen size={16}/> Library</button><button class="icon-button" title="Reload part library" on:click={loadLibrary}><RefreshCw size={16}/></button></div>{#if showLibrary}<div class="library">{#each partGroups as group}<button title={`${group.files.length} program file(s)`} on:click={() => armStoredPart(group)}>{group.label}<span>{group.files.length}</span></button>{:else}<span class="hint">No part folders found.</span>{/each}</div>{/if}<button class:active={placing?.kind === 'hole'} class="btn btn-secondary" on:click={placeHole}><Crosshair size={16}/> Add hole</button>{#if placing}<p class="hint">Hold <kbd>A</kbd> and click to place {placing.label}. Press Esc to cancel.</p>{/if}</section>{#if selected}<section><h2>Selection</h2><strong>{selected.label}</strong><div class="coordinate-grid"><label>X<input type="number" step="0.001" value={selected.x} on:change={(e) => commit(placements.map(item => item.id === selected.id ? { ...item, x: Number(e.currentTarget.value) } : item))}/></label><label>Y<input type="number" step="0.001" value={selected.y} on:change={(e) => commit(placements.map(item => item.id === selected.id ? { ...item, y: Number(e.currentTarget.value) } : item))}/></label></div><div class="row"><button class="icon-button" title="Rotate left 90 degrees" on:click={() => rotateSelected(-1)}><RotateCcw size={17}/></button><button class="icon-button" title="Rotate right 90 degrees" on:click={() => rotateSelected(1)}><RotateCw size={17}/></button><button class="icon-button" title="Duplicate" on:click={duplicateSelected}><Copy size={17}/></button>{#if selected.kind === 'part'}<button class="icon-button" title="Inspect G-code" on:click={inspectSelected}><FileCode size={17}/></button>{/if}<button class="icon-button danger" title="Delete placement" on:click={removeSelected}><Trash2 size={17}/></button></div></section>{/if}<section><h2>Measurement</h2>{#if measure.length === 2}<strong>{Math.hypot(measure[1].x - measure[0].x, measure[1].y - measure[0].y).toFixed(3)} in</strong><button class="text-button" on:click={() => { measure = []; draw(); }}>Clear measurement</button>{:else}<p class="hint">Select the ruler, then click two points.</p>{/if}</section></aside>
