<script>
  ...
  import { Plus, Save, Undo2, Redo2, RotateCw, RotateCcw, Download, Upload, Crosshair, MousePointer2, FolderOpen, FolderInput, Files, Search, RefreshCw, Ruler, FileCode, Trash2, Copy, Settings, X, Pencil, ExternalLink, AlertTriangle, Cpu, ArrowDown, ArrowUp, FolderCog } from 'lucide-svelte';
  ...
  let undo = createUndoStack([]), gcodePrograms = {}, partGroups = [], newSheet = { name: '', width: 48, height: 30, thickness: '0.125' }, editSheet = { id: null, name: '', width: 48, height: 30, thickness: '0.125' }, showEditSheet = false, clipboardPlacement = null, pasteCount = 0;
  ...
  onMount(() => {
    ...
    const onKey = (event) => {
      ...
      if (event.key.toLowerCase() === 'd' && selected && !(event.metaKey || event.ctrlKey)) { event.preventDefault(); duplicateSelected(); }
      if (event.key.toLowerCase() === 'h') { event.preventDefault(); selectNextHole(); }
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'c') { event.preventDefault(); copySelected(); }
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'v') { event.preventDefault(); pasteClipboard(); }
      ...
      if (event.key.toLowerCase() === 'v' && !(event.metaKey || event.ctrlKey)) { event.preventDefault(); showInactiveCuts = !showInactiveCuts; draw(); }
  ...
  function beginEditSheet(item) {
    editSheet = { id: item.id, name: item.name, width: item.width_in, height: item.height_in, thickness: item.thickness_key };
    showEditSheet = true;
  }
  async function saveEditSheet() {
    if (!editSheet.name.trim()) return toastActions.show('Name the sheet first');
    try {
      await updateSheet(editSheet.id, { name: editSheet.name.trim(), width_in: editSheet.width, height_in: editSheet.height, thickness_key: editSheet.thickness });
      showEditSheet = false;
      await refreshSheets();
      toastActions.show(`Saved ${editSheet.name.trim()}`);
  ...
  // Holes are small canvas targets, easy to miss by click - H cycles
  // through every placed hole instead, wrapping back to the first after the
  // last. Starts from the beginning whenever the current selection isn't a
  // hole (nothing selected, or a part is selected).
  function selectNextHole() {
    const holes = placements.filter((item) => item.kind === 'hole');
    if (!holes.length) return;
    const currentIndex = holes.findIndex((item) => item.id === selectedId);
    selectedId = holes[(currentIndex + 1) % holes.length].id;
    draw();
  }
  function copySelected() { if (!selected) return; clipboardPlacement = { ...selected }; pasteCount = 0; }
  function pasteClipboard() { if (!clipboardPlacement) return; pasteCount += 1; const offset = .5 * pasteCount; const copy = makePlacement({ ...clipboardPlacement, id: undefined, x: clipboardPlacement.x + offset, y: clipboardPlacement.y + offset, label: `${clipboardPlacement.label} copy` }); commit([...placements, copy]); selectedId = copy.id; }
...
{#if showOutputEditor}<OutputEditorModal repositoryUrl={JPROG_OUTPUT_REPOSITORY} on:close={() => (showOutputEditor = false)} />{/if}

<style>
  :global(main.container.page-container:has(.workspace)){max-width:none;padding:0}
  .nesting,.workspace{max-width:1400px;margin:0 auto;padding:28px}.nesting header,.workspace-header,.row{display:flex;align-items:center;justify-content:space-between;gap:12px}.eyebrow{margin:0;color:var(--text-muted);font-size:.8rem;text-transform:uppercase;letter-spacing:0}.nesting h1,.workspace h1{margin:2px 0;font-size:1.7rem}.settings-panel{max-width:700px;margin-top:24px;padding:20px;border:1px solid var(--border);border-radius:6px}.settings-panel h2{font-size:1rem;margin:16px 0 4px}.settings-panel h2:first-child{margin-top:0}.settings-panel p,.hint{color:var(--text-muted);line-height:1.45}.header-actions{display:flex;gap:8px;align-items:center}.jprog-identity{display:grid;gap:8px;justify-items:start}.jprog-identity h1{margin:0}.header-sheet-name{display:grid;gap:2px;text-align:center;color:var(--text-muted)}.header-sheet-name strong{color:var(--text);font-size:1rem}.header-sheet-name span{font-size:.8rem}.search{display:flex;margin-top:24px;max-width:540px;align-items:center;gap:8px;border:1px solid var(--border);padding:8px 10px}.search input{border:0;padding:0;min-width:0;width:100%}.sheet-list{margin-top:14px;display:grid;gap:8px;max-width:720px}.sheet-row{display:flex;justify-content:space-between;gap:12px;text-align:left;padding:16px;border:1px solid var(--border);background:var(--surface-1);border-radius:6px}.sheet-row span{color:var(--text-muted)}.workspace{max-width:none;padding:14px;height:calc(100vh - 70px);display:flex;flex-direction:column}.workspace-header{padding:0 4px 14px;border-bottom:1px solid var(--border)}.workspace-body{flex:1;min-height:0;display:grid;grid-template-columns:270px 1fr;margin-top:12px;gap:12px}aside{border:1px solid var(--border);padding:12px;overflow:auto}aside section{display:grid;gap:9px;padding:12px 0;border-bottom:1px solid var(--border)}aside h2{font-size:1rem;margin:0}.sheet-specs{display:grid;gap:2px;color:var(--text-muted);font-size:.85rem}.sheet-specs strong{color:var(--text)}.library{display:grid;gap:4px;max-height:190px;overflow:auto}.library button{padding:7px;text-align:left;border:1px solid var(--border);background:var(--surface-1);display:flex;justify-content:space-between;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}.library button:hover,.library button:focus-visible{transform:translateX(3px);border-color:#2563eb;box-shadow:0 3px 10px #10182822;outline:none}.canvas-wrap{position:relative;min-height:0;background:#000;border:1px solid #344054;overflow:hidden}canvas{width:100%;height:100%;touch-action:none}.canvas-status{position:absolute;bottom:12px;left:12px;color:#fff;background:#101828d9;padding:7px 10px;display:flex;gap:7px;font-size:.8rem}.btn,.text-button{display:inline-flex;align-items:center;justify-content:center;gap:7px}.text-button{border:0;background:none;color:#2563eb;justify-content:start;padding:2px}.upload input{display:none}.active{outline:2px solid #2563eb}.danger{color:#b42318}.selection-actions{display:grid;gap:6px}.selection-actions .btn{justify-content:flex-start}label{display:grid;gap:5px;font-size:.85rem}input,select{padding:8px;border:1px solid var(--border);border-radius:4px;background:var(--surface-1);color:inherit}.coordinate-grid,.two{display:grid;grid-template-columns:1fr 1fr;gap:8px}.scrim{position:fixed;inset:0;background:#10182899;display:grid;place-items:center;z-index:10}.modal{position:relative;background:var(--surface-1);padding:22px;width:min(460px,calc(100vw - 32px));display:grid;gap:14px;border-radius:8px;max-height:calc(100vh - 32px);overflow:auto}.modal h2{margin:0}.modal-close{position:absolute;right:12px;top:12px;border:0;background:none;color:inherit}.modal fieldset{display:grid;gap:8px;border:1px solid var(--border)}.radio{display:flex;align-items:center;gap:8px}.radio input{padding:0}.program{width:min(900px,calc(100vw - 32px))}.program details{border:1px solid var(--border);padding:8px}.program pre{white-space:pre;overflow:auto;max-height:320px;font-size:.75rem}@media(max-width:720px){.workspace{height:auto;min-height:100vh;padding:10px}.workspace-body{grid-template-columns:1fr;grid-template-rows:auto 65vh}.workspace-header{align-items:flex-start}.header-actions{flex-wrap:wrap;justify-content:end}.nesting,.workspace{padding:16px}.sheet-row{display:grid;gap:4px}.canvas-status{max-width:calc(100% - 24px)}}

  .jprog-home { max-width: 1180px; padding-top: 48px; }
  .jprog-home-header { padding-bottom: 26px; border-bottom: 1px solid var(--border); }
  .jprog-home h1 { font-size: 2rem; font-weight: 700; }
  .home-subtitle { margin: 5px 0 0; color: var(--text-muted); }
  ...
  .sheet-index-toolbar > div > span { display: block; margin-top: 3px; color: var(--text-muted); font-size: .82rem; }
  .sheet-index .search { margin: 0; width: min(100%, 380px); max-width: none; background: var(--surface-1); border-radius: 5px; }
  ...
  .open-sheet { color: #2563eb; font-size: .88rem; font-weight: 600; }
  .empty-sheets { display: grid; gap: 5px; padding: 32px 16px; color: var(--text-muted); border: 1px dashed var(--border); text-align: center; }
  ...
  .icon-button { width: 2rem; height: 2rem; padding: 0; border: 1px solid var(--border); border-radius: 4px; background: var(--surface-1); display: inline-flex; align-items: center; justify-content: center; }
  ...
  aside section:nth-child(3) .row button:first-child { outline: 2px solid #2563eb; outline-offset: -2px; }
  ...
  .save-status { color: var(--text-muted); font-size: .78rem; white-space: nowrap; }
  ...
  .workflow-stats { justify-content: flex-start; flex-wrap: wrap; color: var(--text-muted); font-size: .78rem; }
  .workflow-stats span { padding: 3px 6px; border: 1px solid var(--border); border-radius: 999px; }
  ...
  .library-browser { width: min(860px, calc(100vw - 32px)); min-height: min(620px, calc(100vh - 64px)); align-content: start; }
  ...
  .library-browser-item { display: flex; gap: 8px; border: 1px solid var(--border); border-radius: 5px; padding: 8px; }
  ...
  .library-open:hover strong { color: #2563eb; }
  .library-open span { color: var(--text-muted); font-size: .8rem; overflow-wrap: anywhere; }
  ...
  /* Upload AutoCAM / Files both bring outside G-code into this sheet -
     Library previously shared its own icon with Files, making the two
     indistinguishable at a glance. A distinct accent (not the shared
     --primary token, which resolves to white in this app's theme and was
     already confirmed invisible on plain buttons) marks both as "import"
     actions without touching the global .btn-secondary class. */
  .placement-actions .action-import { border-color: #bfdbfe; color: #1d4ed8; }
  .placement-actions .action-import:hover { background: #eff6ff; border-color: #2563eb; color: #2563eb; }
