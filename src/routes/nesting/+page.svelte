<script>
  ...
  let showNewSheet = false, showLibrary = true, showLibraryModal = false, loadingLibraryModal = false, libraryScope = 'all', showCamJobs = false, loadingCamJobs = false, camJobs = [], camJobSearch = '', camJobDateFrom = '', camJobDateTo = '', showEmit = false, showProgram = false, showCommands = false, committingGcode = false, deletingCut = false, activeCutId = null, pendingAutoCamFile = null, user = null, loadError = '', showOutputEditor = false;
...
<style>
  ...
  /* Anchored to .canvas-wrap (position:relative), the same way .canvas-status
     already is - not position:fixed against the viewport. Fixed positioning
     put this at a raw viewport corner regardless of where the canvas itself
     actually ended up on screen, so shrinking the window, going fullscreen,
     or the mobile breakpoint stacking the sidebar above a shorter canvas
     could all land these buttons over unrelated page content instead of the
     canvas's own corner. */
  .workspace-output-actions { position: absolute; right: 20px; bottom: 20px; z-index: 3; display: flex; flex-direction: column; align-items: stretch; gap: 0.5rem; }
