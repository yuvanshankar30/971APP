...
{#if screen === 'edit'}<div class="workspace-output-actions"><button type="button" class="btn btn-secondary" on:click={() => (showOutputEditor = true)}><FolderCog size={16}/> Output Editor</button><a class="btn btn-primary" href={JPROG_OUTPUT_REPOSITORY} target="_blank" rel="noreferrer"><ExternalLink size={16}/> Open Output Repository</a></div>{/if}
...
<style>
  ...
  .workspace-output-actions { position: fixed; right: 28px; bottom: 28px; z-index: 3; display: flex; flex-direction: column; align-items: stretch; gap: 0.5rem; }
