<script>
  ...
  async function removeActiveCut() {
    ...
    try {
      ...
      // Keep deletion responsive: draw the next cut as soon as its previews are ready,
      // then hydrate the other cuts without blocking the editor.
      await loadPlacedPrograms(placements);
      draw();
      void loadPlacedPrograms(renderedPlacements.filter(item => !item.renderActive)).then(draw);
