...
// nesting_sheets also points to its active cut, so PostgREST sees two
// relationships to nesting_cuts. Name the ownership relationship explicitly.
export async function listSheets() { const { data, error } = await supabase.from('nesting_sheets').select('*, nesting_cuts!nesting_cuts_sheet_id_fkey(*)').order('updated_at', { ascending: false }); fail(error); return data || []; }
...
export async function setCutProgramType(cutId, programType) { const { error } = await supabase.from('nesting_cuts').update({ program_extension: programType }).eq('id', cutId); fail(error); }
