...
Tool 1, independent of everything else it does for every other operation.
...
necessarily its approved release-cut tool. Confirmed live via the Fusion
MCP against a real open document: its real "Slot Cut for Edges" operation
really was on Tool 2.

Direct instruction: the release/slot cut is always Tool 1 now, in both
multi-tool and single-tool mode (previously Tool 6 in multi-tool mode) -
Tool 6 is reserved exclusively for genuinely sized (dimensioned/
toleranced) hole operations that aren't also a big-endmill tier, and a
release/slot cut is neither. It must still resolve independently of
filter_guids - the set of tools an operator actually selected for this
part's own geometry, which has no obligation to include Tool 1 for that
reason.
"""
...
_LARGE_ENDMILL_GUID = "29331875-1efc-47c5-9742-f39efcb697ed"  # 6mm, 0.2362in, NOT Tool 1 or 6
...
class ReleaseCutForcedToTool1Tests(unittest.TestCase):
    ...
    def test_new_router_release_cut_gets_tool_1_even_when_filter_guids_excludes_it(self):
        # filter_guids only carries the large endmill the operator selected
        # for this part's own cutting - never Tool 1, exactly like a real
        # job's own tool selection might look.
    ...
    def test_old_router_release_cut_also_gets_tool_1(self):
    ...
    def test_release_cut_is_not_reassigned_by_the_generic_largest_endmill_pass(self):
        ...
        # Tool 1 - if the release cut leaked into the generic multi-tool
        # assignment pass afterward, it would end up reassigned to this one
        # instead, silently overwriting the forced Tool 1 above it.
        root = self._patch({_LARGE_ENDMILL_GUID, _REAL_TOOL_1_GUID})
        release_cuts = _release_cut_templates(root)
        self.assertEqual(release_cuts[0].find("x:tool", _NS).get("guid"), _REAL_TOOL_1_GUID)

    def test_release_cut_is_not_reassigned_even_when_tool_6_is_also_loaded(self):
        # Tool 6 is loaded for this job (a real sized-hole operation needs
        # it), but that must never leak into the release cut - it stays on
        # Tool 1 regardless of what else is in the tool library.
        root = self._patch({_LARGE_ENDMILL_GUID, _REAL_TOOL_1_GUID, _REAL_TOOL_6_GUID})
        release_cuts = _release_cut_templates(root)
        self.assertEqual(release_cuts[0].find("x:tool", _NS).get("guid"), _REAL_TOOL_1_GUID)

    # Direct instruction: the release/slot cut always requires Tool 1, in
    # both multi-tool and single-tool mode.
    ...
    def test_single_tool_mode_release_cut_falls_back_to_tool_1_when_tool_6_was_selected(self):
        # Tool 6 is no longer an eligible release-cut tool at all, even
        # when it's the job's one single selected tool - the release cut
        # must still fall back to Tool 1, not use Tool 6.
        root = self._patch({_REAL_TOOL_6_GUID, _REAL_TOOL_1_GUID}, multi_tool_mode=False, single_tool_number=6)
        release_cuts = _release_cut_templates(root)
        self.assertEqual(release_cuts[0].find("x:tool", _NS).get("guid"), _REAL_TOOL_1_GUID)
    ...
    def test_single_tool_mode_release_cut_falls_back_to_tool_1_when_selected_tool_is_neither(self):
        ...
        # not NC number 1) - the release cut must fall back to Tool 1, not
        # stay on the large endmill.
        root = self._patch({_LARGE_ENDMILL_GUID}, multi_tool_mode=False, single_tool_number=2)
        release_cuts = _release_cut_templates(root)
        self.assertEqual(release_cuts[0].find("x:tool", _NS).get("guid"), _REAL_TOOL_1_GUID)

    def test_multi_tool_mode_ignores_single_tool_number_and_still_requires_tool_1(self):
        ...
        root = self._patch({_LARGE_ENDMILL_GUID, _REAL_TOOL_1_GUID}, multi_tool_mode=True, single_tool_number=6)
        release_cuts = _release_cut_templates(root)
        self.assertEqual(release_cuts[0].find("x:tool", _NS).get("guid"), _REAL_TOOL_1_GUID)

    def test_raises_when_no_tool_1_exists_in_single_tool_mode_with_no_explicit_selection(self):
        ...
        # number falls back to requiring Tool 1. Tool 1 doesn't exist in
        # this library (only Tool 3), so it must still raise.
    ...
    def test_raises_when_no_tool_1_exists_in_any_loaded_library_in_multi_tool_mode(self):
        # Multi-tool mode keeps the fixed Tool 1 requirement unconditionally
        # - see resolve_required_release_cut_tool_number's own comment. A
        # real preset is required here (unlike the single-tool-mode fixture
        ...
        library_json = json.dumps({
            ...
            "data": [
                {
                    "description": "6mm for toolchanger",
                    "type": "flat end mill",
                    "guid": _LARGE_ENDMILL_GUID,
                    "post-process": {"number": 6},
                    "geometry": {"DC": 0.2362},
                    "start-values": {
