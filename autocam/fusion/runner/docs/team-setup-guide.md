...
**Run `setup.py` once; you should never have to manually update this machine again.** On every add-in start and every five minutes while it is idle, the Runner asks the authenticated Hub update API for the current release manifest. When the deployed Runner revision differs, it downloads the release pinned to that manifest, verifies its SHA-256 checksum, and replaces the Runner code, templates, tools, and postprocessors - straight from the deployed app, no `git` involved at all. It always preserves this workstation's `.env`, `.overridepath`, `deps`, and `temp` folders. The release marker is written last, so an interrupted update is retried rather than mistaken for a completed one. After an update is staged, the Runner stops claiming jobs and asks for Fusion to be fully quit and reopened so its Python process can load the new modules safely. A failed update check is logged and retried later without preventing an otherwise configured Runner from processing jobs.

For box tube, the Runner creates four manually indexed setups (Sides 12, 3, 6, and 9), each on **G55** with X along the tube and the origin on the machined wall's right-hand corner at one end. Every side gets a tube cutoff at the far end, so all four produce NC files; the cutoff stops 0.16" short of each corner, leaving the corners as tabs to break after the last side. See [`tubestock-cam.md`](tubestock-cam.md) for the full tube workflow. Verify the setup side, toolpath direction, and near-wall-only breakthrough in Fusion before cutting a new tube/template combination.
