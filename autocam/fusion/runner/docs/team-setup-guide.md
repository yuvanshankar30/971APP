...
1. **Copy the add-in into Fusion's AddIns folder, named exactly `SpartanRoboticsAutoCAM`:**
   ```bash
   ...
   ```
   (Windows: `%APPDATA%\Autodesk\Autodesk Fusion 360\API\AddIns\SpartanRoboticsAutoCAM`. The name must match exactly, no spaces - Fusion loads it as a Python package and won't list it otherwise.)

   Or download the ready-made zip from the [Fusion AutoCAM Setup page](/autocam/fusion/setup) and unzip it there instead - it's already named correctly.
