...
- **Tool #5 is claimed by two different countersinks** (0.372in and 0.5in, both 82°), so the database trigger permits only one loaded record per machine at a time. The queue UI and Runner deliberately allowlist both exact library GUIDs, but surface and use only whichever record is currently loaded. This keeps the 0.5in option available when it is the deliberate physical swap without treating both T5 records as simultaneously installed.
