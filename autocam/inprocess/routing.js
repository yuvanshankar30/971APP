 ...
 *     change pause, M3/M5 spindle, M30 program end, "(...)" comments. M-codes
 *     are unpadded (M3, not M03) and spindle-off is always paired with S0
 *     (S0 M5) - JProg (jprog/), which also parses this app's output for
 *     sheet nesting, aborts the whole file on a leading-zero M-code or a
 *     bare M5 with no S word on the same line.
 *
 ...
 *       - M3/M5 (spindle on/off) ARE kept for 'wincnc' - not because
...
export function generateRoutingGcode(contours, params = {}) {
  ...
  if (isWinCNC) {
    ...
    // G49 (cancel tool length offset) deliberately omitted from this cancel
    // line - JProg (the shop's sheet-nesting tool, see jprog/) doesn't
    // recognize it and aborts the whole file on an unrecognized G-code, and
    // this program never issues G43 in the first place, so there's no
    // offset of its own to cancel; the G80/G40 cancels stay for the same
    // other-programs-leftover-state insurance.
    lines.push('G80 G40 (cancel canned cycle / cutter comp - defensive, in case a prior program on this machine left one active)');
  }
  if (edgeMargin > 0) {
  ...
  if (!hasSequence) {
    ...
    lines.push(`S${spindleSpeed} M3 (spindle on)`);
      ...
      for (let toolIndex = 0; toolIndex < toolSequence.length; toolIndex += 1) {
        ...
        if (toolIndex !== currentToolIndex) {
          if (currentToolIndex === -1) {
            ...
            lines.push(`S${tool.spindleSpeed} M3 (spindle on)`);
            if (spindleDwellSeconds > 0) lines.push(dwellLine(isWinCNC, spindleDwellSeconds, 'wait for spindle to reach speed'));
            lines.push(`G00 Z${fmt(safeZ)} (safe height)`);
          } else {
            toolChanges += 1;
            ...
            lines.push(`S${tool.spindleSpeed} M3 (spindle back on)`);
            if (spindleDwellSeconds > 0) lines.push(dwellLine(isWinCNC, spindleDwellSeconds, 'wait for spindle to reach speed'));
            lines.push(`G00 Z${fmt(safeZ)} (safe height)`);
          }
