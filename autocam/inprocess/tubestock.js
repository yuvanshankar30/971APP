...
function generateProgram(walls, params, { faceAngleDeg = null, faceLabel = null, programNumber, cutoffFeature = null, profileFeatures = null }) {
  ...
  if (isWinCNC) {
    ...
    lines.push('G55 (tube stock fixture work offset - the tube fixture lives here, not in G54)');
    // G49 (cancel tool length offset) deliberately omitted - JProg (the
    // shop's sheet-nesting tool, see jprog/) doesn't recognize it and aborts
    // the whole file on an unrecognized G-code, and this program never
    // issues G43 in the first place, so there's no offset of its own to
    // cancel; G80/G40 stay for the other-programs-leftover-state insurance.
  ...
  groups.forEach((group, toolIndex) => {
    if (toolIndex === 0) {
      ...
      lines.push(`S${spindleSpeed} M3 (spindle back on)`);
      if (spindleDwellSeconds > 0) lines.push(dwellLine(isWinCNC, spindleDwellSeconds, 'wait for spindle to reach speed'));
    }
