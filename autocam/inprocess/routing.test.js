...
describe('generateRoutingGcode - single-tool (default)', () => {
  ...
  it('states the tool diameter to load before the spindle starts (no ATC on these routers - no T-code/M06 - but the operator still needs to know what to load)', () => {
    ...
    const m3Idx = lines.findIndex((l) => /\bM3\b/.test(l));
    expect(toolLineIdx).toBeGreaterThan(-1);
    expect(toolLineIdx).toBeLessThan(m3Idx); // stated before the spindle ever starts, not after
  ...
  it('uses the carbide 0.1575-inch 971 Main Bit Fusion defaults and never enables coolant', () => {
    const result = generateRoutingGcode(contour, { toolDiameter: 0.25, targetDepth: 0.25 });
    expect(result.gcode).toContain('S22000 M3');
...
describe('generateRoutingGcode - controller dialect (default linuxcnc vs wincnc for the ShopSabre Pro 408)', () => {
  ...
  it('wincnc dialect uses a bare G4 (not M00) for the tool-change pause, still keeps M3/M5', () => {
    const toolSequence = [
      { toolDiameter: 0.25, toolNumber: 1, label: '1/4in roughing bit' },
      { toolDiameter: 0.125, toolNumber: 2, label: '1/8in detail bit' }
    ];
    ...
    expect(result.gcode).toContain('S0 M5');
  });
  ...
  it('linuxcnc header defensively cancels canned cycle/cutter comp (in case a prior program on the shared machine left one active); wincnc omits it, unconfirmed against WinCNC\'s own manual; G49 omitted on both - JProg cannot parse it and this program never issues G43', () => {
    const linuxcnc = generateRoutingGcode(contour, { toolDiameter: 0.25, targetDepth: 0.25 });
    expect(linuxcnc.gcode).toMatch(/^G80 G40\b/m);
    expect(linuxcnc.gcode).not.toContain('G49');
    const wincnc = generateRoutingGcode(contour, { toolDiameter: 0.25, targetDepth: 0.25, controller: 'wincnc' });
...
describe('generateRoutingGcode - spindle spin-up dwell (real risk: engaging the cut before the spindle reaches commanded RPM is a stall/broken-tool risk, not theoretical)', () => {
  ...
  it('defaults to a 2-second dwell (G04 P, linuxcnc) right after the initial spindle-on', () => {
    const result = generateRoutingGcode(contour, { toolDiameter: 0.25, targetDepth: 0.25 });
    const lines = result.gcode.split('\n');
    const m3Index = lines.findIndex((l) => /\bM3\b/.test(l));
    expect(m3Index).toBeGreaterThan(-1);
    expect(lines[m3Index + 1]).toMatch(/^G04 P2\.0 /);
  });
  ...
  it('multi-tool mode dwells after every M3, including mid-program tool-change restarts', () => {
    const toolSequence = [
      { toolDiameter: 0.25, toolNumber: 1, label: '1/4in roughing bit' },
      { toolDiameter: 0.125, toolNumber: 2, label: '1/8in detail bit' }
    ];
    const result = generateRoutingGcode(partWithHoles(), { targetDepth: 0.25, toolSequence });
    const m3Count = (result.gcode.match(/\bM3\b/g) || []).length;
    const dwellCount = (result.gcode.match(/G04 P2\.0/g) || []).length;
    expect(m3Count).toBeGreaterThan(1); // real tool change actually happened
    expect(dwellCount).toBe(m3Count); // every spindle-on got its own dwell
  });
});
