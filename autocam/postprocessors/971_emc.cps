...
properties = {
  ...
  preloadTool: false, // preloads next tool on tool change if any, TMS 12/28/2014 changed from true
  showSequenceNumbers: false, // show sequence numbers, TMS 12/28/2014 changed from true
...
var mapCoolantTable = new Table(
  [9, null, 7, null, null, null, 8, null],  //TMS 8-15-2015, Changed from [9,8,7] so that commanding flood will result in error, and mist and vacuum are supported
...
function onOpen() {
  ...
  //Initiate G64 Path Blending -Steinwerks Jan 16, 2018
  writeBlock(gFormat.format(64), "P" + xyzFormat.format(.001), "Q" + xyzFormat.format(.001));
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    onDwell(10); //TMS 2014-12-28 Added to try to get some dwell
  }
  ...
  // set coolant after we have positioned at Z
  {
    ...
    if (tool.coolant == 7) {  //#TMS 8-15-2015 adding support for mist and vacuum
      writeBlock(mFormat.format(7));  //TMS 8-15-2015 turn on mist
      writeBlock(mFormat.format(8));  //TMS 8-15-2015 turn on vacuum
    } else {  //TMS 8-15-2015 Otherwise handle like they do
      if (c) {
        writeBlock(mFormat.format(c));
      } else {
        error(localize("Coolant not supported."));  //TMS 8-15-2015 changed warning to error
      }
    }
  }
...
var mapCommand = {
  ...
  COMMAND_LOAD_TOOL:6,
  COMMAND_COOLANT_ON:8,  //TMS 2014-12-28 This does not change the default coolant
