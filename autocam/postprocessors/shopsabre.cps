...
wcsDefinitions = {
  useZeroOffset: true,
  wcs          : [
    {name:"Standard", format:"G", range:[0, 6]}
  ]
};
...
function onSection() {
  ...
  // 0/1 is G54
  if (workOffset > 1) {
    var workOffsetCode = 53 + workOffset;
    if (workOffsetCode <= 59) {
      writeBlock(gFormat.format(workOffsetCode));
    } else {
       error(localize("Work offset out of standard G54-G59 range."));
       return;
    }
  }
