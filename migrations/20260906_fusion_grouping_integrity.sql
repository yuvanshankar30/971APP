...
BEGIN;
ALTER TABLE public.cam_jobs ADD COLUMN IF NOT EXISTS fusion_nc_files jsonb;
COMMENT ON COLUMN public.cam_jobs.fusion_nc_files IS
  'Separate byte-preserved files emitted by the configured Fusion postprocessor: [{name, contentBase64, size, sha256}].';
DO $$ BEGIN
  IF EXISTS (
