...
- `GEMINI_MODEL` is optional and defaults to `gemini-2.5-flash-lite`.
...
double-post from another server instance. Gemini has an eight-second request
timeout; if Slack retries while a reply is still processing, the durable
receipt acknowledges the duplicate without posting again. The model has no
mutation tools and cannot change Hub data.
